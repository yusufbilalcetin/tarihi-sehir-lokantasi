...
test("the cockpit is a service home screen with a four-tab bar, not three panes", () => {
  // The three-pane stand layout is gone: no media-query branch decides the
  // shape, and no menu rail or standing table column exists for it to decide.
  assert.doesNotMatch(cockpit, /TABLET_MEDIA_QUERY/, "the pane-switching media query came back");
  assert.doesNotMatch(cockpit, /aria-label="Menü ve masa filtreleri"/, "the left rail came back");
  assert.doesNotMatch(cockpit, /<ProductBrowser/, "the middle menu pane came back");
  assert.doesNotMatch(cockpit, /aria-label="Seçili masa"/, "the standing table column came back");
  // What replaced it is one home canvas plus a bottom bar.
  assert.match(cockpit, /<OperationalHome role="service"/);
  assert.match(cockpit, /<ServiceHome/);
...
test("the room widens with the screen instead of splitting into fixed panes", () => {
  // One board, more of it per row as the width allows. No fixed-width column
  // is carved out of a 768px tablet any more.
  assert.match(cockpit, /grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5/);
  assert.doesNotMatch(cockpit, /w-52[^"]*(?:md|lg):flex/, "the left rail came back");
  assert.doesNotMatch(cockpit, /w-\[19rem\]|w-\[21rem\]/, "a fixed pane came back");
  // The table's own detail is progressive disclosure, at every width.
  assert.match(cockpit, /<Sheet open=\{Boolean\(parts\.selectedTable\)\}/);
  assert.match(cockpit, /side="bottom"/);
});

test("the home scrolls as one page and only the sheet scrolls inside itself", () => {
  // A home screen is a page, not a set of locked panes.
  assert.doesNotMatch(cockpit, /h-\[calc\(100dvh-8rem\)\]/, "the page was locked to the viewport again");
  assert.match(cockpit, /min-h-0 flex-1 overflow-y-auto/, "the table sheet body cannot scroll");
  assert.match(cockpit, /h-\[92dvh\]/, "the sheet lost its phone height");
});
...
test("touch targets on every waiter control clear 44px", () => {
  ...
  for (const [name, source] of [
    ...
    // The card is itself the target (min-h-32); the queues use 64px rows.
    assert.match(
      source,
      /min-h-(?:32|16|14|11)/,
  ...
  // The home launcher's own tiles are the largest targets on the screen.
  const operational = read("components/staff/operational-ui.tsx");
  assert.match(operational, /min-h-\[84px\]/, "a launcher tile dropped under 44px");
  assert.match(operational, /size-14 shrink-0/, "the launcher icon shrank");
});
...
test("the redesign moved the markup and left the behaviour where it was", () => {
  ...
  // The order pad went with the rest of the mutations: table-grid decides when
  // a round may still be added to, and the composer owns the replay key, with
  // the same discipline, so a double tap cannot double-charge.
  assert.match(tableGrid, /canAddItemsToOrder\(toApiOrderStatus\(selectedOrder\.status\)\)/);
  assert.match(tableGrid, /<TableOrderComposer/);
  const composer = read("components/staff/table-order-composer.tsx");
  assert.match(composer, /idempotencyKeyRef\.current \?\?= staffApi\.newIdempotencyKey\(\)/);
  assert.match(composer, /idempotencyKeyRef\.current = null;/);
  assert.doesNotMatch(cockpit, /staffApi\./, "the home forked an order mutation of its own");
});
