...
test("kitchen and cashier wire snapshot tracking to their dedicated sounds", () => {
  ...
  const operationalChrome = read("components/staff/operational-ui.tsx");
  ...
  assert.match(kitchen, /<OperationalTopBar[\s\S]*sound/);
  assert.match(cashier, /<OperationalTopBar[\s\S]*sound/);
  assert.match(operationalChrome, /import \{ SoundControl \}/);
  assert.match(operationalChrome, /sound \? <SoundControl \/> : null/);
});
