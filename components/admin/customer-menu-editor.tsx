...
type ArchiveTarget =
  | { readonly kind: "product"; readonly item: AdminProductResult }
  | { readonly kind: "category"; readonly item: AdminCategoryResult; readonly productCount: number };
...
export function CustomerMenuEditor({ focus = "all" }: { readonly focus?: MenuEditorFocus }) {
  ...
  // The restaurant's own name, from the session that already carries it. A
  // literal here would keep showing the old name the day the owner renames it.
  const { restaurantName: sessionRestaurantName } = useStaffSession();
  const restaurantName = sessionRestaurantName ?? "Menü";
  ...
  }, [draft.categories, draft.products, restaurantName, showHidden]);
  ...
  async function confirmArchive() {
    ...
    const result = archiveTarget.kind === "product"
      ? await menu.run(
          () => adminApi.updateProduct(archiveTarget.item.id, { archived: true }),
          `${archiveTarget.item.name} arşivlendi.`,
        )
      : await menu.run(
          () => adminApi.updateCategory(archiveTarget.item.id, { archived: true }),
          `${archiveTarget.item.name} kategorisi arşivlendi.`,
        );
    // A failed request leaves both the confirmation and the editor open. The
    // manager can retry without losing the form or mistaking failure for done.
    if (!result) return;
    if (archiveTarget.kind === "product") setEditingProductId(null);
  ...
  if (menu.loading && draft.categories.length === 0) {
    return <LoadingState rows={6} variant="grid" className="p-4" />;
  }
  ...
  if (menu.error && draft.categories.length === 0) {
    return (
      <ErrorState
        ...
        description="Menü verileri alınamadı. Kayıtlı içeriğe dokunulmadı."
  ...
  return (
    ...
    >
      {menu.error ? (
        <div role="alert" className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-status-danger/25 bg-status-danger-tint/40 px-4 py-3">
          <p className="text-sm font-semibold text-status-danger">Menü yenilenemedi; son alınan içerik gösteriliyor.</p>
          <Button type="button" size="sm" variant="outline" onClick={() => void menu.refetch()}>Tekrar dene</Button>
        </div>
      ) : null}
      {/* The negative margin cancels the page container's padding so this bar
          bleeds to the content edge. It therefore has to mirror that padding at
          every step, and the container's last step is `lg:px-8` — it has no
          `xl` rule. An `xl:-mx-10` here pulled 8px per side further than there
          was padding to cancel, which is a real 8px horizontal overflow on
          every screen from 1280px up, hidden from the eye by the shell's
          `overflow-x-clip` but still present in `scrollWidth`. */}
      <div className="sticky top-[var(--admin-topbar)] z-[var(--z-appbar)] -mx-4 mb-2 flex h-[var(--menu-editor-bar)] items-center gap-2 border-b bg-card/95 px-4 backdrop-blur supports-[backdrop-filter]:bg-card/80 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
        ...
        <div className="ms-auto flex items-center gap-1.5">
          <Button
            ...
            variant="ghost"
            size="sm"
            className="min-h-11 min-w-11 px-0 text-xs text-foreground/75 sm:px-3"
      ...
      */}
      ...
      {editingProduct ? (
        <ProductEditDialog
          ...
          onArchived={() => setArchiveTarget({ kind: "product", item: editingProduct })}
        />
      ) : null}
      ...
      <Dialog open={archiveTarget !== null} onOpenChange={(open) => !open && !menu.saving && setArchiveTarget(null)}>
        <DialogContent showCloseButton={!menu.saving}>
          <DialogHeader>
            <DialogTitle>
              {archiveTarget?.kind === "product" ? "Ürünü arşivle" : "Kategoriyi arşivle"}
            </DialogTitle>
            <DialogDescription>
              {archiveTarget?.kind === "product"
                ? `${archiveTarget.item.name} müşteri menüsünden kaldırılacak; geçmiş siparişlerdeki kaydı korunacak.`
                : archiveTarget
                  ? `${archiveTarget.item.name} menüden kaldırılacak; içindeki ${archiveTarget.productCount} ürün silinmeyecek.`
                  : "Bu işlem menü görünürlüğünü değiştirir."}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button type="button" variant="outline" disabled={menu.saving} onClick={() => setArchiveTarget(null)}>
              Vazgeç
            </Button>
            <Button type="button" variant="destructive" disabled={menu.saving} aria-busy={menu.saving} onClick={() => void confirmArchive()}>
              {menu.saving ? "Arşivleniyor…" : "Arşivle"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function ProductOverlay({
  ...
  return (
    <>
      ...
      <span className="absolute end-1.5 top-1.5 z-10 flex items-center gap-0.5 opacity-0 transition-opacity duration-[var(--motion-quick)] group-hover/edit:opacity-100 group-focus-within/edit:opacity-100 [@media(hover:none)]:opacity-100">
        {reorderable ? (
          <>
            <Button
              type="button"
