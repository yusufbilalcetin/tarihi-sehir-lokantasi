      ...
      <div className="pointer-events-none relative flex min-w-0 flex-col p-[var(--menu-card-padding)]">
        <div>
          <div className="flex flex-wrap gap-1.5">
            ...
            {soldOut ? <Badge variant="outline" className="border-order-void/25 bg-order-void-tint px-1.5 py-0 text-[10px] font-semibold text-order-void">{t("soldOut")}</Badge> : null}
          </div>
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-3">
          ...
          <Button type="button" size="icon" disabled={soldOut || !canOrder} onClick={onAdd} aria-label={`${name}: ${t("addToCart")}`} className="pointer-events-auto relative z-10 size-11 rounded-xl shadow-sm">
