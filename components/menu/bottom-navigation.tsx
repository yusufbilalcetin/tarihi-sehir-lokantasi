...
export function BottomNavigation({ active, cartCount, onChange }: { active: MenuTab; cartCount: number; onChange: (tab: MenuTab) => void }) {
  ...
  return (
    <nav dir={direction} aria-label={t("customerNavigation")} className="fixed inset-x-0 bottom-0 z-40 border-t bg-card/96 pb-[env(safe-area-inset-bottom)] shadow-[0_-12px_36px_rgba(37,33,29,0.1)] backdrop-blur-xl">
      <div className="mx-auto grid h-16 w-full max-w-xl grid-cols-4 px-2">
        {items.map(({ id, labelKey, icon: Icon }) => (
          <button
            ...
            onClick={() => onChange(id)}
            className={cn(
              "relative grid h-16 min-w-0 grid-rows-[1.25rem_1rem] place-items-center content-center gap-1 rounded-xl px-1 text-center text-[11px] font-semibold leading-4 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
          ...
          >
            ...
            {id === "order" && cartCount > 0 ? <span className="absolute end-[20%] top-1.5 flex min-w-4 items-center justify-center rounded-full bg-burgundy px-1 text-[9px] font-bold leading-4 text-white">{formatNumber(cartCount)}</span> : null}
          </button>
        ))}
      </div>
    </nav>
