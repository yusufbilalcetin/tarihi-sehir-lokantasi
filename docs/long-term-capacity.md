...
**Historical measurement on a disposable TEST database**, taken with only
migrations `0000`–`0009` applied and no business data. It is *not* a reading of
production, and the two figures below are not current production values:
production today has a **20-row** migration ledger and **61** `public` base
tables (`0000`–`0019` applied). The 10/20 pair below is kept only because the
size figures it accompanies were measured at that schema version.
