...
- `docs/data-retention.md` — 61 tablonun tamamının sınıflandırması
  (`lib/config/data-retention.ts`), otomatik silme allow/deny listesi ve
  maintenance servisi.
...
## TARİHSEL: 0013–0016 rollout runbook'u (uygulanmıştır)

> **Bu bölüm tarihseldir ve gelecekteki bir iş değildir.** `0013`–`0016`
> production'a **uygulanmıştır**; sonrasında `0017`, `0018` ve `0019` da
> uygulanmıştır. Güncel durum (salt-okunur doğrulama, 2026-08-31, PostgreSQL
> 17.6): migration ledger **20 satır** (`0000`–`0019`, bekleyen yok) ve `public`
> şemasında **61 base table**.
>
> Bölüm, runner davranışı, kilit profili ve yedek/doğrulama disiplini
> gelecekteki her rollout için hâlâ geçerli olduğu için korunmuştur. Aşağıdaki
> adımlar ve sayılar `0013`–`0016` batch'inin **o zamanki** durumunu anlatır;
> bugünün beklenen değerleri olarak okunmamalıdır.
...
| Outbox cron | **Supabase → Cron → ilgili işi duraklat** | Zamanlama Supabase tarafında yönetilir; `vercel.json` cron tanımlamaz. Migration'ın kilitlediği tablolara dokunmaz, ancak pencere boyunca duraklatılmalıdır |
...
2. Yeni yazma işlemleri durduruldu (`restaurants.is_active = false`, Supabase outbox cron işi duraklatıldı)
...
-- migration geçmişi 0016'ya kadar (o zamanki beklenti: 17 satır).
-- Bugünkü doğrulanmış değer 20 satırdır (0000-0019 uygulanmıştır).
