...
| **Automatic deletion deny-list** | the other **58** tables (`NEVER`) |
...
`lib/config/data-retention.ts` declares a policy for **all 61 tables** — one
entry per `pgTable` across `db/schema.ts` (31) and `db/erp-schema.ts` (30), with
no table left unclassified. That file, not this one, is the complete
classification.

**The prose below does not enumerate all 61.** It walks 26 of them: the core
operational, financial, master-data and technical tables. The **35 it does not
list in a table** are the 30 Phase 38 ERP tables in `db/erp-schema.ts` and the
five printing tables (`printer_agents`, `restaurant_printers`, `printer_routes`,
`print_jobs`, `print_job_attempts`). Every one of those 35 is classified
`NEVER` in the source — the ERP set under one shared reason ("deleting them
would change a historical answer or break live master-data references") and the
printing set individually. None of them is on the auto-delete allow-list, so the
"one rule" above still holds for them without restating each. Read the source
for the per-table purpose and reason.
...
| `cashier_shift_cash_counts` | The physically counted drawer, one row per denomination and phase: what a named cashier counted, in which currency, at which face value | `denominations × currencies × phases × shifts_per_day × 365 × years` |
...
`cashier_shift_cash_counts` (added by migration `0018`) is the evidence behind
an opening float and a closing variance. Deleting a row would leave a shift
total that nothing in the system can account for, and cash discrepancies are
investigated long after the day ends — so its policy is `NEVER`, like the shift
it belongs to.
...
| `category_translations` | follows its category | Localized category display names/descriptions; live master data that follows the category lifecycle through the tenant-safe foreign key |
| `product_translations` | follows its product | Localized product display names/descriptions; live master data that follows the product lifecycle through the tenant-safe foreign key |
...
The two translation tables (added by migration `0019`) carry **no history
dependency**: a sold line snapshots the product name it was sold under on
`order_items`, so localized content is presentation master data, not a
financial record. They are nevertheless `NEVER` auto-deleted — the source
declares no deletion semantics for them, and none is invented here. Their
lifecycle is the foreign key to their category/product, not a maintenance job.
Current production content: `category_translations` 656 rows,
`product_translations` 5252 rows (read-only reading, 2026-08-31).
