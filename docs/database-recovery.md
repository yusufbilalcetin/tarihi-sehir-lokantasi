...
**Verified reading, 2026-08-31** (read-only, production): ledger 17 rows against
20 journal entries — `0017`, `0018` and `0019` pending; 58 base tables against
61 declared, the three missing ones being exactly `cashier_shift_cash_counts`
(added by `0018`) and `category_translations` + `product_translations` (added by
`0019`). `0017` adds a nullable column, not a table. The two counts agreeing
this way is itself the check: 58 + 3 = 61.
