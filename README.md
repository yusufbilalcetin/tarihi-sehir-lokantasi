...
Bu çağrının zamanlaması **bu depoda tutulmaz**: `vercel.json` hiçbir cron
tanımlamaz. Zamanlama Supabase altyapısında yönetilir ve iş
`/api/internal/outbox/dispatch` adresine
`Authorization: Bearer <OUTBOX_DISPATCH_SECRET>` başlığıyla `POST` gönderir.
İş kurulu değilse uygulama sorunsuz yanıt vermeye devam eder; yalnızca realtime
durur ve outbox büyür.

Route hem `POST` hem `GET` kabul eder, ikisi de aynı bearer kontrolünden geçer:
Supabase Cron `POST` kullanır, `GET` yalnızca `GET`-only zamanlayıcılarla
uyumluluk için durur. Secret olmadan gelen çağrı 401 alır.
