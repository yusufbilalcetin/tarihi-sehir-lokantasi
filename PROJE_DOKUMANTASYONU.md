...
├── vercel.json              Dakikalık outbox cron'u
...
- Migration'lar: `db/migrations/0000...0012.sql`.
- ORM: Drizzle.
...
`vercel.json`, `/api/internal/outbox/dispatch` endpoint'ini her dakika çağırır. Vercel Cron `GET` ve `Authorization: Bearer $CRON_SECRET` kullandığı için:

- route hem `GET` hem `POST` kabul eder,
- deployment'ta `CRON_SECRET`, `OUTBOX_DISPATCH_SECRET` ile aynı olmalıdır,
- bu endpoint düzenli çağrılmazsa realtime yayını durur ve kuyruk büyür,
- route sabiti çağrı başına en fazla 20 olay işler; dakikalık cron kullanıldığında
  tek deployment için nominal üst sınır dakikada 20 olaydır.
...
| `CRON_SECRET` | Server-only | Vercel cron bearer secretı |
...
4. `CRON_SECRET` ve `OUTBOX_DISPATCH_SECRET` değerini Vercel Cron kontratına göre aynı ayarlayın.
