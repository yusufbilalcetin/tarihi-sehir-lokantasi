...
const logger = createLogger("api.admin.tables.qr.revoke");
...
export async function POST(
  ...
  try {
    ...
    const revoked = await createTableService().revokeToken(principal, {
    ...
    if (failure.status >= 500) {
      logger.error("revoke_failed", "Restaurant table QR could not be revoked.", {
