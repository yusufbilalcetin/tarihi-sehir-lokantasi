...
const logger = createLogger("api.orders.items.cancel");

/** Soft-cancels one line and rewrites the order totals in the same transaction. */
export async function POST(
  ...
  try {
    ...
    const result = await service.cancelItem(principal, {
    ...
    if (failure.status >= 500) {
      logger.error("cancel_item_failed", "Order item could not be cancelled.", {
